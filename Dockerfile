...
RUN apt-get update && apt-get install -y wget unzip curl gnupg jq libglib2.0-0 libnss3 libgconf-2-4 libfontconfig1 libxss1 libappindicator3-1 libasound2 libxtst6 libatk-bridge2.0-0 libgtk-3-0 libx11-xcb1 libxcb-dri3-0 libgbm1 xdg-utils --no-install-recommends && \
...
RUN CHROME_VERSION=$(google-chrome --version | grep -oP '\d+\.\d+\.\d+\.\d+') && \
    ...
    curl -sSL "$DRIVER_VERSION" -o /tmp/chromedriver.zip && \
    unzip /tmp/chromedriver.zip -d /usr/local/bin/ && \
    chmod +x /usr/local/bin/chromedriver && \
    rm /tmp/chromedriver.zip
